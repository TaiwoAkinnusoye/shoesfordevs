...
const mocks = [
    {
        ...
        result: {data: {
            currentUser: {...fakeUser(), cart: [fakeCartItem({id: 'abc123'})]}
        }}
    },
    {
...
describe('<RemoveFromCart />', () => {
    ...
    it('removes an item from the cart', async () => {
        ...
        expect(res.data.currentUser.cart).toHaveLength(1);
        expect(res.data.currentUser.cart[0].item.price).toBe(5000);

        wrapper.find('button').simulate('click');
        await wait();
        const res2 = await apolloClient.query({query: CURRENT_USER_QUERY});
        expect(res2.data.currentUser.cart).toHaveLength(0);
    })
})